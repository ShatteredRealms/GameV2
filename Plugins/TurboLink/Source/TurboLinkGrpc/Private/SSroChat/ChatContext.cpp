...
void GrpcContext_ChatService_ConnectDirectMessage::Call(const FGrpcSroCharacterCharacterTarget& Request)
{
	check(GetState() == EGrpcContextState::Ready);
	UpdateState(EGrpcContextState::Initialing);
...
void GrpcContext_ChatService_GetAuthorizedChatChannels::Call(const FGrpcSroCharacterCharacterTarget& Request)
{
	check(GetState() == EGrpcContextState::Ready);
	UpdateState(EGrpcContextState::Initialing);
...
GrpcContext_ChatService_SetUserChatChannelAuthorizations::GrpcContext_ChatService_SetUserChatChannelAuthorizations(FGrpcContextHandle _Handle, UGrpcService* _Service, UGrpcClient* _Client)
	: Super(_Handle, _Service, _Client)
{
}

void GrpcContext_ChatService_SetUserChatChannelAuthorizations::Call(const FGrpcSroChatRequestSetChannelAuth& Request)
{
	check(GetState() == EGrpcContextState::Ready);
	UpdateState(EGrpcContextState::Initialing);

	::sro::chat::RequestSetChannelAuth rpcRequest;
	TURBOLINK_TO_GRPC(&Request, &rpcRequest);

	UChatService* service = (UChatService*)Service;
	RpcReaderWriter = service->d->Stub->AsyncSetUserChatChannelAuthorizations(RpcContext.get(), rpcRequest, service->TurboLinkManager->d->CompletionQueue.get());
	RpcReaderWriter->ReadInitialMetadata(InitialTag);
}

void GrpcContext_ChatService_SetUserChatChannelAuthorizations::OnRpcEvent(bool Ok, const void* EventTag)
{
	Super::OnRpcEventInternal(Ok, EventTag, 
		[this](const FGrpcResult& _Result, ::google::protobuf::Empty* _RpcResponse) 
		{
			UChatServiceClient* client = (UChatServiceClient*)(this->Client);
			if (!(client->OnSetUserChatChannelAuthorizationsResponse.IsBound())) return;

			FGrpcGoogleProtobufEmpty response;
			if (_RpcResponse) {
				GRPC_TO_TURBOLINK(_RpcResponse, &response);
			}
			client->OnSetUserChatChannelAuthorizationsResponse.Broadcast(Handle, _Result, response);
		}
	);
}
