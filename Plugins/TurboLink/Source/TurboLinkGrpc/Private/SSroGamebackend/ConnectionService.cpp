...
void UConnectionService::CallConnectGameServer(const FGrpcSroCharacterCharacterTarget& Request, FConnectGameServerResponseLambda ResponseLambda, TMap<FString, FString>& MetaData)
{
...
void UConnectionService::CallVerifyConnect(const FGrpcSroGamebackendVerifyConnectRequest& Request, FVerifyConnectResponseLambda ResponseLambda, TMap<FString, FString>& MetaData)
{
	if (InnerClient == nullptr)
	{
		InnerClient = MakeClient();
...
void UConnectionService::CallTransferPlayer(const FGrpcSroGamebackendTransferPlayerRequest& Request, FTransferPlayerResponseLambda ResponseLambda, TMap<FString, FString>& MetaData)
{
	if (InnerClient == nullptr)
	{
		InnerClient = MakeClient();
...
void UConnectionService::CallIsPlaying(const FGrpcSroCharacterCharacterTarget& Request, FIsPlayingResponseLambda ResponseLambda, TMap<FString, FString>& MetaData)
{
	if (InnerClient == nullptr)
	{
		InnerClient = MakeClient();
	}
	UConnectionServiceIsPlayingLambdaWrapper* lambdaWrapper = NewObject<UConnectionServiceIsPlayingLambdaWrapper>();
	lambdaWrapper->InnerClient = InnerClient;
	lambdaWrapper->ResponseLambda = ResponseLambda;
	lambdaWrapper->Handle = InnerClient->InitIsPlaying();
	InnerClient->OnIsPlayingResponse.AddUniqueDynamic(lambdaWrapper, &UConnectionServiceIsPlayingLambdaWrapper::OnResponse);
	InnerClient->IsPlaying(lambdaWrapper->Handle, Request, MetaData);
}
